...
final class KeywordProvider
{
    ...
    public function getKeyword(string $title, int $extSystemId): string
    {
        ...
        $id = $this->defaultConnection->fetchOne(
            'SELECT id FROM keyword WHERE name = :title AND ext_system_id = :ext_system_id',
            [
                'title' => $title,
        ...
        if (false === is_string($id)) {
            $id = $this->createKeyword($title, $extSystemId);
        }
     ...
     * @throws Exception
     */
    private function createKeyword(string $title, int $extSystemId): string
    {
        $id = (string) Uuid::v6();
