...
{
    private const string TTS_SLOT_FREE = 'free';
    ...
    private function setAudioProperties(AssetCmsSysDto $dto, Asset $asset): void
    {
        ...
        $this->setTtsProperties($dto, $asset);
    }

    /**
     * Populate TTS-specific fields when the asset was generated by TTS and is currently active.
     */
    private function setTtsProperties(AssetCmsSysDto $dto, Asset $asset): void
    {
        $tts = $this->ttsAssetRepository->findByAsset($asset);
        if (null === $tts || TtsAudioStatus::Active !== $tts->getStatus()) {
            return;
        }

        $dto->setGenerated(true);
        $dto->setVoiceFamilySlug($tts->getVoiceFamilySlug());
        $dto->setExtResourceName($tts->getExtResourceName());
        $dto->setExtId($tts->getExtId());
        $dto->setExtVersion($tts->getExtVersion());
        $dto->setIncludeInRecommendedPodcast($tts->isIncludeInRecommendedPodcast());
        $dto->setLastRegeneratedAt($tts->getLastRegeneratedAt());
        $dto->setLicenceId($asset->getLicence()->getId());

        $freeSlot = $asset->getSlots()->findFirst(
            static fn (mixed $key, AssetSlot $slot): bool => self::TTS_SLOT_FREE === $slot->getName()
        );
        $previewAudio = $freeSlot instanceof AssetSlot ? $freeSlot->getAssetFile() : null;
        if ($previewAudio instanceof AudioFile) {
            $dto->setPreviewMediaUrl($previewAudio->getMainRoute()?->getUri()->getPath());
        }
    }
