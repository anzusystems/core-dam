...
final class Version20240313134157 extends AbstractMigration
{
    public function up(Schema $schema): void
    {
        $idConsole = App::getUserIdConsole();
        $scraperHash = PasswordHelper::passwordHash('TOKEN_sys_cms');
        ...
        VALUES
            (100016, 'sys_anzu_core_cms.anzu@smeonline.sk', 'Cms', 'Cms', 'Cms', '#9D1508', 'Cms', '[\"ROLE_SYS_API\"]',
            '{\"dam_asset_create\":2,\"dam_asset_view\":2}', 0, '{$idConsole}', '{$idConsole}', NOW(), NOW(),
            JSON_ARRAY(), JSON_ARRAY(), '{$scraperHash}'
         ) ON DUPLICATE KEY UPDATE id = id
        ");

        $this->addSql('INSERT INTO users_to_ext_systems (user_id, ext_system_id) VALUES (100016, 1)');
    }

    public function down(Schema $schema): void
    {
        $this->addSql('DELETE FROM user WHERE id = 100006');
    }
}
