...
final class Version20240222135915 extends AbstractMigration
{
    ...
    {
        $this->addSql('
            CREATE TABLE asset_licence_in_group (
                asset_licence_id INT NOT NULL,
                asset_licence_group_id INT NOT NULL,
                INDEX IDX_4C135576E00EE493 (asset_licence_id),
                INDEX IDX_4C1355769572D68A (asset_licence_group_id),
                PRIMARY KEY(asset_licence_id, asset_licence_group_id)
            ) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        ');
        $this->addSql("
            CREATE TABLE asset_licence_group (
                ...
                INDEX IDX_563A0204E961F7A (ext_system_id),
                INDEX IDX_563A0204B03A8386 (created_by_id),
                INDEX IDX_563A020499049ECE (modified_by_id),
                UNIQUE INDEX UNIQ_name_ext_system (name, ext_system_id),
                PRIMARY KEY(id)
            ) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        ");
        $this->addSql('
            CREATE TABLE job_podcast_synchronizer (
                id INT NOT NULL,
                podcast_id VARCHAR(36) NOT NULL,
                full_sync TINYINT(1) NOT NULL,
                PRIMARY KEY(id)
            ) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        ');
        $this->addSql('
            CREATE TABLE job_user_data_delete (
                id INT NOT NULL,
                target_user_id INT NOT NULL,
                anonymize_user TINYINT(1) NOT NULL,
                PRIMARY KEY(id)
            ) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        ');
        $this->addSql('
            CREATE TABLE user_in_licence_groups (
                user_id INT NOT NULL,
                asset_licence_group_id INT NOT NULL,
                INDEX IDX_1789A906A76ED395 (user_id),
                INDEX IDX_1789A9069572D68A (asset_licence_group_id),
                PRIMARY KEY(user_id, asset_licence_group_id)
            ) DEFAULT CHARACTER SET utf8mb4 COLLATE `utf8mb4_unicode_ci` ENGINE = InnoDB
        ');

        $this->addSql(
            'INSERT INTO job_podcast_synchronizer (id, podcast_id, full_sync)
                SELECT id, podcast_id, full_sync from job where discr = \'jobpodcastsynchronizer\''
        );

        $this->addSql(
            'INSERT INTO job_user_data_delete (id, target_user_id, anonymize_user)
                SELECT id, target_user_id, anonymize_user from job where discr = \'jobuserdatadelete\''
        );
        $this->addSql('
            ALTER TABLE job
                ADD scheduled_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT \'(DC2Type:datetime_immutable)\',
                ADD priority SMALLINT DEFAULT 0 NOT NULL,
                DROP podcast_id, DROP full_sync,
                DROP target_user_id,
                DROP anonymize_user,
                CHANGE discr discriminator VARCHAR(255) NOT NULL
        ');
